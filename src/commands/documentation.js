...
module.exports = {
    ...
    async execute(interaction) {
        const embedRegles = new EmbedBuilder()
            ...
            `Avoir participé à l'énigme te donne le droit de faire \`/jouer\`. Tu pourras lancer un dé (1 à 6) et avancer sur le plateau.\n\n` +
            `**3. L'Étoile**\n` +
            `L'Étoile se trouve sur une case du plateau. Si tu passes devant ou t'arrêtes dessus, tu peux l'acheter pour **20 pièces**. Elle se déplace ensuite aléatoirement ailleurs.`);
        ...
        const embedCases = new EmbedBuilder()
            ...
            `👻 **Boo** : Permet de voler des pièces (gratuit) ou une Étoile (coûte 50 pièces) à un autre joueur.\n` +
            `🔥 **Bowser** : Événement catastrophique (perte de moitié des pièces, perte d'étoile, révolution communiste, etc.).\n` +
            `🎭 **Coup du Sort** : Événement global aléatoire (échange de places, loterie, duel de dés, etc.).\n` +
            `🛒 **Boutique** : Permet d'acheter des objets avec tes pièces.`);
