...
function createTimeout(userId, type, interaction) {
    ...
    j.temp_choix_direction = defaultPath;
                ...
                } catch(e) { console.error(e); } finally { const { unlockUser } = require('../../game/transaction'); unlockUser(userId); }
        }
    }, 60000);
}
...
async function handleUseItem(interaction) {
    try {
        ...
        if (item.id === 'champignon') {
            ...
            joueur.position = (() => { const v = require('../../game/board').BOARD_CASES.filter(ca => ca.id <= 45 && ca.type !== 'Boutique' && ca.type !== 'Boo' && ca.id !== 1).map(ca => ca.id); return v[Math.floor(Math.random() * v.length)]; })();
            ...
            const plateau = await Plateau.findByPk(1);
            const boardDef = require('../../game/board').BOARD_CASES;
